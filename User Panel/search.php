...
$maxBudget = intval($_GET['budget'] ?? 35000);
...
$filteredProperties = array_filter($allProperties, function($p) use ($searchQuery, $selectedCity, $selectedType, $selectedGender, $maxBudget, $verifiedOnly) {
    ...
    if (!empty($searchQuery)) {
        ...
        $matches = str_contains(strtolower($p['title']), $q) || 
                   str_contains(strtolower($p['area']), $q) || 
                   str_contains(strtolower($p['type']), $q);
      ...
      <div class="col-lg-3">
        <div class="card border-0 rounded-4 shadow-sm bg-white p-4 sticky-top" style="top: 85px; z-index: 100;">
          ...
          <form action="search.php" method="GET">
            ...
            <div class="mb-3">
              <label class="form-label extra-small fw-bold text-secondary mb-1">Popular Amenities</label>
              <div class="form-check small mb-1">
                <input class="form-check-input" type="checkbox" id="am1" checked>
                <label class="form-check-label extra-small" for="am1">High-Speed Wi-Fi</label>
              </div>
              <div class="form-check small mb-1">
                <input class="form-check-input" type="checkbox" id="am2" checked>
                <label class="form-check-label extra-small" for="am2">3-Time Meals (Food)</label>
              </div>
              <div class="form-check small mb-1">
                <input class="form-check-input" type="checkbox" id="am3">
                <label class="form-check-label extra-small" for="am3">Air Conditioned (AC)</label>
              </div>
              <div class="form-check small mb-1">
                <input class="form-check-input" type="checkbox" id="am4">
                <label class="form-check-label extra-small" for="am4">Attached Washroom</label>
              </div>
            </div>
          ...
          <?php if (empty($filteredProperties)): ?>
            <div class="col-12 text-center py-5">
              <div class="card border-0 rounded-4 bg-soft-lavender p-5 max-w-md mx-auto">
                ...
                <a href="search.php" class="btn btn-nh-primary btn-sm px-4 mx-auto">Reset Filters</a>
              </div>
            </div>
          <?php else: ?>
                  ...
                  <div class="p-3 d-flex flex-column flex-grow-1">
                    <div class="d-flex justify-content-between align-items-center mb-1">
                      ...
                      <span class="badge bg-warning text-dark extra-small flex-shrink-0">
                        <i class="fas fa-star me-1"></i> <?= htmlspecialchars($item['rating']) ?> (<?= $item['reviews_count'] ?>)
                      </span>
                    </div>
              ...
              <div class="row g-3 align-items-center">
                <div class="col-md-4">
                  <div class="rounded-3 overflow-hidden position-relative" style="height: 160px;">
                    <img src="<?= htmlspecialchars($item['image']) ?>" alt="<?= htmlspecialchars($item['title']) ?>" class="w-100 h-100 object-fit-cover" />
                ...
                <div class="col-md-5">
                  <div class="d-flex align-items-center gap-2 mb-1">
                    ...
                    <?php if ($item['verified']): ?><span class="badge bg-success extra-small">Verified</span><?php endif; ?>
...
<script>
  ...
  const propertiesJson = <?= json_encode($filteredProperties) ?>;

  function switchView(mode) {
    const gridView = document.getElementById("searchGridContainer");
    const listView = document.getElementById("searchListContainer");
    const mapView = document.getElementById("searchMapContainer");
  ...
  function initSearchMap() {
    ...
    propertiesJson.forEach(p => {
      if (p.lat && p.lng) {
        const marker = L.marker([p.lat, p.lng]).addTo(searchLeafletMap);
        marker.bindPopup(`
    ...
    if (bounds.length > 0) {
      searchLeafletMap.fitBounds(bounds, { padding: [50, 50] });
    }
  }
</script>
