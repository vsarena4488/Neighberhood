<?php
// Owner Panel messages.php - 2-Column Responsive Chat Engine & Quick Reply Templates
$pageTitle = 'Tenant Messages · NeighborNest Owner Console';
require_once __DIR__ . '/includes/functions.php';

$chats = &$_SESSION['owner_chats'];
...
$pageTitle = 'Messages & Tenant Inquiries · NeighborNest';
require_once __DIR__ . '/includes/header.php';
require_once __DIR__ . '/includes/sidebar.php';
?>

<style>
  /* Owner Messages Custom Styles */
  .chat-card-container {
  ...
  .chat-item:hover { background: var(--nh-bg-light); }
  .chat-item.active { background: var(--nh-soft-lavender); }
...
<div class="main-wrapper">
  ...
  <main class="page-content">
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
      <div>
        <h3 class="fw-bold mb-1">Messages & Tenant Inquiries</h3>
        <span class="text-secondary-custom small">Direct communication with verified student tenants and prospective applicants</span>
      </div>
    </div>
            ...
            <?php foreach ($chats as $ch): ?>
              <a href="messages.php?chat=<?= $ch['id'] ?>" class="chat-item <?= ($ch['id'] === $activeChat['id']) ? 'active' : '' ?>">
                <div class="position-relative flex-shrink-0">
                  <img src="<?= htmlspecialchars($ch['user_avatar']) ?>" alt="<?= htmlspecialchars($ch['user_name']) ?>" class="chat-avatar" />
                </div>

                <div class="flex-grow-1 overflow-hidden">
                  <div class="d-flex justify-content-between align-items-center mb-1">
                    <strong class="small text-dark text-truncate fw-bold"><?= htmlspecialchars($ch['user_name']) ?></strong>
        ...
        <div class="col-md-7 col-lg-8 d-flex flex-column h-100 bg-light <?= ($mobileView === 'list') ? 'd-none d-md-flex' : 'd-flex' ?>">
          ...
          <div class="p-3 bg-white border-bottom d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center gap-2.5">
              ...
              <img src="<?= htmlspecialchars($activeChat['user_avatar']) ?>" alt="<?= htmlspecialchars($activeChat['user_name']) ?>" class="chat-avatar" style="width: 42px; height: 42px;" />
              <div>
                <strong class="d-block small text-dark fw-bold lh-1 mb-1"><?= htmlspecialchars($activeChat['user_name']) ?></strong>
                <span class="extra-small text-success fw-semibold"><i class="fas fa-circle fs-xs me-1"></i> Verified Student Tenant</span>
              </div>
            </div>

            <!-- Property Reference Badges -->
            <div class="d-flex align-items-center gap-2">
              <span class="btn btn-sm btn-light border extra-small text-truncate d-none d-sm-inline-flex align-items-center gap-1.5 px-3 py-1.5 rounded-pill" style="max-width: 200px;">
          ...
          <!-- QUICK REPLIES BAR -->
          <div class="px-3 py-2 bg-light border-top d-flex gap-2 overflow-x-auto" style="scrollbar-width: none;">
            <span class="quick-reply-pill" onclick="insertQuickReply('Yes, 200 Mbps fiber Wi-Fi is 100% free and included.')">⚡ Wi-Fi Included</span>
            <span class="quick-reply-pill" onclick="insertQuickReply('Physical room visits are allowed between 2 PM and 6 PM tomorrow.')">⚡ Visiting Hours</span>
            <span class="quick-reply-pill" onclick="insertQuickReply('Delicious North and South Indian meals are served 3 times daily.')">⚡ Meal Info</span>
            <span class="quick-reply-pill" onclick="insertQuickReply('Gate curfew is 10:30 PM with biometric card entry.')">⚡ Curfew Info</span>
          </div>

          <!-- Chat Input Bar -->
          <div class="p-3 bg-white border-top">
            <form action="messages.php?chat=<?= $activeChat['id'] ?>" method="POST" class="d-flex align-items-center gap-2.5">
              <button type="button" class="btn btn-light border rounded-circle p-2.5" title="Attach Photos" onclick="alert('Attachment feature simulated.')">
                <i class="fas fa-paperclip text-muted"></i>
              </button>
              <input type="text" name="message_text" id="messageInput" class="form-control rounded-pill px-3 py-2 extra-small" placeholder="Type your message or click a quick reply..." required autocomplete="off" />
